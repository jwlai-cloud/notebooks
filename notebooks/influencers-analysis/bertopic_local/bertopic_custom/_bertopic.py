...
class BERTopic:
    ...
    def __init__(
        ...
        embedding_model: Union[
            str, SentenceTransformer, DocumentEmbeddings, TokenEmbeddings
        ] = None,
    ...
    def fit_transform(
        self, documents: List[str], embeddings: np.ndarray = None
        ...
        if not any(
            ...
            embeddings = self._extract_embeddings(
                documents.Document, verbose=self.verbose
            )
    ...
    def find_topics(
        self, search_term: str, top_n: int = 5
        ...
        sims = cosine_similarity(
            search_embedding.reshape(1, -1), self.topic_embeddings
        ).flatten()
    ...
    def get_topic_freq(self, topic: int = None) -> Union[pd.DataFrame, int]:
        ...
        if isinstance(topic, int):
            ...
            return pd.DataFrame(
                self.topic_sizes.items(), columns=["Topic", "Count"]
    ...
    def visualize_topics(self):
        ...
        words = [
            " | ".join([word[0] for word in self.get_topic(topic)[:5]])
        ...
        embeddings = umap.UMAP(
            n_neighbors=2, n_components=2, metric="hellinger"
    ...
    def save(self, path: str, save_embedding_model: bool = True) -> None:
    ...
    def _extract_embeddings(
        self, documents: Union[List[str], str], verbose: bool = None
    ) -> np.ndarray:
            ...
            for index, document in tqdm(enumerate(documents), disable=not verbose):
                try:
                    sentence = (
                        Sentence(document)
                        if document
                        else Sentence("an empty document")
                    )
    ...
    def _reduce_dimensionality(
        self, embeddings: Union[np.ndarray, csr_matrix]
    ) -> np.ndarray:
    ...
    def _extract_topics(self, documents: pd.DataFrame):
        ...
        doc_ids = np.asarray(
            [
                idx
                for idx in np.arange(len(documents_))
                if documents_.iloc[idx]["Topic"] == -1
            ]
        )
        ...
        documents_per_topic = documents_.groupby(["Topic"], as_index=False).agg(
            {"Document": " ".join}
        )
    ...
    def _extract_words_per_topic(self, words: List[str]):
        ...
        self.topics = {
            label: [(words[j], c_tf_idf[i][j]) for j in indices[i]][::-1]
        ...
        if not self.custom_embeddings:
            ...
            for topic, topic_words in self.topics.items():
                ...
                topic_embedding = self._extract_embeddings(
                ...
                self.topics[topic] = [
                    (word, value)
                    for word, value in self.topics[topic]
    ...
    def _select_embedding_model(self) -> Union[SentenceTransformer, DocumentEmbeddings]:
        ...
        # Select embedding model based on language
            ...
            if self.language.lower() in ["English", "english", "en"]:
                ...
                raise ValueError(
                    f"{self.language} is currently not supported. However, you can "
    ...
    def _reduce_to_n_topics(self, documents: pd.DataFrame) -> pd.DataFrame:
        ...
        while len(self.get_topic_freq()) > self.nr_topics + 1:
            ...
            # Update Topic labels
            documents.loc[
                documents.Topic == topic_to_merge, "Topic"
            ] = topic_to_merge_into
    ...
    def _map_probabilities(
        self, probabilities: Union[np.ndarray, None]
    ...
    @staticmethod
    def _plotly_topic_visualization(df: pd.DataFrame, topic_list: List[str]):
        ...
        def get_color(topic_selected):
            if topic_selected == -1:
                ...
                marker_color = [
                    "red" if topic == topic_selected else "#B0BEC5"
    ...
    def _preprocess_text(self, documents: np.ndarray) -> List[str]:
        ...
        if self.stop_words is not None:
            cleaned_documents = [
                remove_stopwords(doc, self.stop_words) for doc in cleaned_documents
            ]
        if self.replace_dic is not None:
            cleaned_documents = [
                replace_words(doc, self.replace_dic) for doc in cleaned_documents
            ]
    ...
    def get_most_relevant_documents(
        self, cluster_id: int, condensed_tree: hdbscan.plots.CondensedTree
    ):
